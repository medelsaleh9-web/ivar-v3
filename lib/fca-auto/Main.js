...
async function handleLoginFailure() {
    logger.Warning("Login failed. Checking AutoLogin settings...");
    
    if (global.Fca.Require.FastConfig.AutoLogin) {
        logger.Normal("AutoLogin is enabled. Attempting to create new appState via fblogin.js...");
        try {
            const loginResult = await FacebookLogin.login();
            if (loginResult.status === true) {
                logger.Success("New appState created successfully. Restarting bot...");
                ...
                logger.Error(`Login via fblogin.js failed: ${loginResult.message}. Please check the saved account credentials.`);
                process.exit(0);
            }
        } catch (err) {
            logger.Error("Unexpected error during auto re-login.", err);
            process.exit(0);
        }
    } else {
        logger.Error("Login failed. Please update appState or enable AutoLogin in FastConfigFca.json.");
        process.exit(0);
    }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    async function clearWarning(retries = 0) {
        try {
            ...
            if (data.data?.fb_scraping_warning_clear?.success) {
                logger.Normal("Account warning cleared successfully!");
                logger.Normal("Restarting bot...");
    ...
    if (needWarningHandle) {
        clearWarning().catch(e => logger.Warning("Cannot clear account warning:", e));
    }
        ...
        rl.question(typePasswordPrompt, function (Password) {
            rl.question(type2FAPrompt, function (Secret) {
                ...
                try {
                    ...
                    logger.Warning(Language.ErrDataBase || "Cannot save login credentials.");
                    logger.Error();
                    process.exit(0);
                }
