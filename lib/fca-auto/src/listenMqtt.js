...
function buildStream(options, WebSocket, Proxy) {
    ...
    WebSocket.onerror = (err) => {
        console.log('Error: WebSocket.onerror', err);
        clearTimers();
        Stream.destroy(err);
    };
...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
    ...
    global.mqttClient.on('error', (err) => {
        ...
        if (errMsg.includes('puback') || errMsg.includes('header flag bits')) return;
        console.log('Error: mqttClient.on(error)', err);
        ...
        if (mqttReconnectCount < maxReconnectAttempts) {
            ...
            globalCallback({
                type: 'stop_listen',
                error: 'Server disconnected - Auto Restart'
            }, null);
...
module.exports = function(defaultFuncs, api, ctx) {
    ...
    return function(callback) {
        ...
        if (!ctx.firstListen || !ctx.lastSeqId) {
            ...
            console.log("[LOG] lastSeqId found, calling listenMqtt directly...");
            listenMqtt(defaultFuncs, api, ctx, globalCallback);
        }
